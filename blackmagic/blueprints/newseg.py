...
newseg = Blueprint('newseg', __name__)
...
def skip_on_exception(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if get('exception', first(args), None) is None:
            logger.debug('skip_on_exception is running the function')
            return fn(*args, **kwargs)
        else:
            logger.debug('skip_on_exception is skipping the function')
            return first(args)
...
@newseg.route('/segment', methods=['POST'])
def segments():
    ...
    return thread_first(request.json,
                        ...
                        partial(exception_handler, http_status=500, name='nodata-check', fn=partial(nodata_check, cfg=cfg)),
